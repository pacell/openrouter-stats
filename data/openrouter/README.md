...
Two layouts for the same rows:

| | | |
|---|---|---|
| `data/history/<table>/<date>.csv.gz` | **committed** | append-only partitions, one per data date |
| `data/openrouter/<table>.csv` | **gitignored** | consolidated, rebuilt on demand |

```bash
python3 scripts/openrouter_consolidate.py   # rebuild the CSVs from partitions
python3 scripts/openrouter_history_report.py  # what history currently holds
```

The consolidated CSVs are not committed because rewriting them daily would add a
fresh compressed copy of every changed table to git history on every run — about
16 MB a commit, 6 GB a year. The partitions cost under a megabyte a day, because
a run only rewrites the dates it fetched, and only when their contents actually
changed.

Partitions overlap for the last few days, since a day is still settling when
first fetched. On read, the newest partition's version of a row wins; rows the
API has stopped returning are kept.

**This is also how the 8-day series grow.** OpenRouter retains only 8 days of
cache-hit, throughput, latency and error-rate data. The daily job
(`.github/workflows/daily.yml`, 06:00 UTC) appends each day's partition, so the
accumulated record reaches back further than the API will serve — the spans in
the inventory below are what existed at seeding and lengthen from there.

## Table inventory

Generated by `scripts/openrouter_inventory.py` from the files themselves, so it
cannot drift from the data. "Grain" is the finest key each table is stored at.

**Time series**

| table | grain | rows | span | periods | models | endpoints |
|---|---|---|---|---|---|---|
| `effective_prices_daily_by_endpoint` | model x provider | 128,107 | 2026-01-23 .. 2026-09-01 | 222 | 396 | 1223 |
| `effective_prices_daily_by_model` | model | 55,482 | 2026-01-23 .. 2026-09-01 | 222 | 396 | — |
| `listed_price_changes` | model x provider | 21,245 | 2025-07-28 .. 2026-09-01 | 221 | 400 | 1236 |
| `provider_token_daily` | provider | 38,093 | 2026-06-04 .. 2026-09-01 | 90 | — | — |
| `blended_price_daily_by_model` | model | 10,654 | 2026-08-02 .. 2026-09-01 | 31 | 395 | — |
| `token_mix_daily_by_model` | model | 10,724 | 2026-08-02 .. 2026-09-01 | 31 | 396 | — |
| `cache_hit_rate_daily_by_endpoint` | model x provider | 13,993 | 2026-08-25 .. 2026-09-01 | 8 | 409 | 1218 |
| `performance_daily_by_endpoint` | model x provider x colo x pctile | 36,404 | 2026-08-25 .. 2026-09-01 | 8 | 343 | 1142 |
| `structured_output_error_rate_daily` | model x provider | 9,345 | 2026-08-25 .. 2026-09-01 | 8 | 329 | 831 |
| `tool_call_error_rate_daily` | model x provider | 11,453 | 2026-08-25 .. 2026-09-01 | 8 | 344 | 996 |
| `endpoint_uptime_hourly` | model x provider | 75,968 | 2026-08-29 .. 2026-09-01 | 4 | 331 | 1151 |
| `endpoint_uptime_daily` | model x provider | 7,137 | 2026-08-30 .. 2026-09-01 | 3 | 408 | 1224 |
| `model_uptime_recent` | model | 60,610 | 2026-08-31 .. 2026-09-01 | 2 | 418 | — |

**Static (current snapshot)**

| table | grain | rows | span | periods | models | endpoints |
|---|---|---|---|---|---|---|
| `benchmark_scores` | model x provider | 3,074 | — | — | 194 | 856 |
| `effective_prices_summary` | model x provider | 1,217 | — | — | 396 | 1217 |
| `endpoint_catalogue` | model x provider | 1,358 | — | — | 412 | 1228 |
| `endpoint_price_tiers` | model x provider | 1,266 | — | — | 66 | 148 |
| `endpoint_pricing_raw` | model x provider | 6,632 | — | — | 412 | 1228 |
| `model_catalogue` | model | 418 | — | — | 418 | — |
| `model_price_headline` | model | 396 | — | — | 396 | — |
| `provider_catalogue` | provider | 105 | — | — | — | — |
| `provider_summary` | provider | 74 | — | — | — | — |
| `top_apps_by_model` | model | 1,660 | — | — | 332 | — |
| `top_colos_by_model` | model x colo | 6,958 | — | — | 374 | — |

### Reading it

- **`model x provider`** means one row per model per provider endpoint. A model
  served by 32 providers has 32 rows per period. Provider name and slug are on
  every such row — no join needed to see who served the traffic.
- **`endpoint_id` is the provider key**, and it is per *model*: the same provider
  serving two models has two endpoint ids. It is also finer than the provider
  itself — Google Vertex appears as separate global/US/Europe endpoints, and
  `provider_slug` collapses those.
- **Periods** counts distinct days (or hours, for `endpoint_uptime_hourly`), not
  calendar coverage: a model that launched last week has rows only from then.
- **Static tables are a snapshot**, overwritten on each run. They carry no
  history, so to track how the catalogue or a listed price changes you need
  either repeated runs or `listed_price_changes`, which is the one static-looking
  concept the API does expose as history.

### What is per model per provider, over time

Everything the site shows per provider, you have:

| what | table | span |
|---|---|---|
| effective input/output price | `effective_prices_daily_by_endpoint` | **222 days** |
| listed price changes | `listed_price_changes` | 221 change dates, back to 2025-07 |
| cache hit rate | `cache_hit_rate_daily_by_endpoint` | 8 days |
| throughput, TTFT, time-to-last-token | `performance_daily_by_endpoint` | 8 days, x4 percentiles |
| tool-call error rate | `tool_call_error_rate_daily` | 8 days |
| structured-output error rate | `structured_output_error_rate_daily` | 8 days |
| uptime | `endpoint_uptime_hourly` / `endpoint_uptime_daily` | ~4 days hourly, ~3 daily |
| benchmark scores | `benchmark_scores` | static |

Only the price history is long. Everything else the site plots per provider is
an 8-day window on OpenRouter's side, so a longer history has to be accumulated
by running this on a schedule.

### What exists per provider but not per model

`provider_token_daily` — 90 days of tokens served, the longest volume history
available, but broken out by that provider's top 9 models plus an `Others`
bucket rather than exhaustively.

### What is model-level only

`model_uptime_recent`, `token_mix_daily_by_model`, `blended_price_daily_by_model`
and `model_price_headline`. The API publishes no per-provider split for token
volumes or sequence lengths, so the blend and the token mix cannot be computed
per provider — that is a limit of the source, not of this pipeline.
